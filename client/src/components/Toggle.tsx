...
function useEffectAfterMount(cb: React.EffectCallback, dependencies: React.DependencyList) {
...
function ToggleOn({ children }: React.PropsWithChildren<{}>): React.ReactElement | null {
  const { on } = useToggleContext()
  return on ? (children as React.ReactElement) : null
}

function ToggleOff({ children }: React.PropsWithChildren<{}>): React.ReactElement | null {
  const { on } = useToggleContext()
